...
let Slide_Counter = 1
display_slide(Slide_Counter);

function Next_slide(x){
    display_slide(Slide_Counter += x)
}

function Current_slide(x) {
    display_slide(Slide_Counter = x)
}
function display_slide(x) {
    let i;
    let Slides = document.getElementsByClassName("Slides")
    if (x > Slides.length) {Slide_Counter = 1}
    if (x < 1) {Slide_Counter = Slides.length}
    for (i = 0; i < Slides.length; i++ ) {
        Slides[i].style.display = "none";
    }
    Slides[Slide_Counter-1].style.display = "inline"
}